...
export default function EstadoPedidosAdmin() {
    ...
    }, [estadoActivo, cargarPedidos, rangoFecha, fechaDesde, fechaHasta]);
    ...
    const handleConfirmarAprobacion = async (observaciones: string) => {
        if (modalAprobacion.pedido) {
            try {
                if (modalAprobacion.pedido.iid_estado_pedido === 6) {
                    await pedidosActions.ejecutarAprobacionCotizacion(modalAprobacion.pedido, observaciones);
                    setMensajeExito('La cotización ha sido aprobada exitosamente');
                } else if (modalAprobacion.pedido.iid_estado_pedido === 1) {
                ...
                Alert.alert('Error', 'Error al aprobar. Por favor intente nuevamente.');
            }
        }
    };
